...
> ⚠️ **Note:** Unlike `rclone`, `s3cmd` uses a single configuration file (`~/.s3cfg`) and is typically tied to one endpoint at a time. If you need to work with multiple endpoints, you must use separate config files (via `--config`) or reconfigure `s3cmd`.
