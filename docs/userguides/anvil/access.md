...
Anvil is an ACCESS computing resource. To use Anvil, you must first create an ACCESS account and request an allocation through the [ACCESS Allocation Request System :octicons-link-external-16:](https://allocations.access-ci.org/).

!!! tip "New users to existing projects"
    If you are joining an existing project, all you need to do is [login or create your ACCESS account :octicons-link-external-16:](https://allocations.access-ci.org/) and the manager of your project can add you themselves!

<hr style="border: 3px solid">

### What is ACCESS?

[Advanced Cyberinfrastructure Coordination Ecosystem: Services & Support (ACCESS) :octicons-link-external-16:](https://access-ci.org) is an NSF-funded program that manages access to the national research cyberinfrastructure (CI) resources. Any researcher who seeks to use one of these CI resources must follow ACCESS processes to get onto these resources.

### What resources are available via ACCESS?

ACCESS coordinates a diverse set of resources including Anvil and other traditional HPC resources suited for resource-intensive CPU workloads, modern accelerator-based systems (e.g., GPU), as well as cloud resources. **Anvil provides both CPU and GPU resources as part of ACCESS.** A comprehensive list of all the ACCESS-managed resources can be found here along with their descriptions and ideal workloads: [https://allocations.access-ci.org/resources :octicons-link-external-16:](https://allocations.access-ci.org/resources).
...
!!! note "How are Anvil usernames assigned?"
    When you receive an Anvil allocation through ACCESS, your Anvil account is automatically created using your ACCESS username.
    Your Anvil username (1) is based on your ACCESS username and (2) uses the same name, with an `x-` prefix added

    For example, ACCESS username: `doug@access-ci.org`  
      - Anvil username: `x-doug`
...
7. For other questions you may have, take a look at [ACCESS policies :octicons-link-external-16:](https://allocations.access-ci.org/allocations-policy).
...
- **Check the Help page when you have issues:** [Help](../help)
