---
title: Gene prediction using Helixer
tags:
  - Life Sciences
  - Annotation
  - Tutorials
...
!!! warning
    As you may have noticed, the number of `mRNA` and `gene` features are the same. This is because isoforms aren't predicted by Helixer and you only have one transcript per gene. Exons are identified with high confidence and alternative isoforms are usually collapsed into a single gene model.
...
The code used for parsing `featureCounts` summary files and generating the plots can be found here: [`parse_subreads.R`](../../assets/scripts/lifesciences/parse_subreads.R)
...
=== "Percent identity"
    ...
    ![pid](../../assets/images/lifesciences/pid.png)
...
The code used for parsing EnTAP's `entap_results.tsv` files and generating the plots can be found here: [`parse_subreads.R`](../../assets/scripts/lifesciences/entap_results_plot.R)

!!! note
    The functional annotation results show that the Helixer predictions have higher proportion of genes with functional annotations compared to the reference annotations. Predictions with similarity to existing sequences in the databases are likely to be more accurate than those without any similarity.
...
The code used for parsing phylostrata analyses ([`phylostrata_run.R`](../../assets/scripts/lifesciences/phylostrata_run.R)) and for plotting results ([`phylostrata_plot.R`](../../assets/scripts/lifesciences/phylostrata_plot.R)) are provided in the repository.
...
### H. CDS assessments (GC, length, start/stop)

We can use `bioawk` get relevant stats on the CDS sequences predicted by Helixer and compare them with the reference annotations.
...
The plots were generated using the following R script: [`cds_assesment.R`](../../assets/scripts/lifesciences/cds_assesment.R)
